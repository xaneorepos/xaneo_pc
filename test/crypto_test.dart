...
void main() {
  ...
  group('CryptoService E2EE Tests', () {
    ...
    test(
      'generateAndStoreKeys creates valid flat-format key bundle with signature',
      () async {
        ...
        expect(
          blob['pub']['ed25519'],
          equals(cryptoService.ed25519PublicKeyHex),
        );
      },
    );
    ...
    test('unlockFromBlob handles legacy Argon2id/XSEC-2 format gracefully', () async {
      final argon2idBlob = {
        ...
        'algorithm': 'XSEC-2',
        'signature':
            'c67a1d0e7a723977bf4264e774b090ed13a3a2a978ea780b78ce2fdc5ea8c5449d5859f0bc8556cb206bb84d09a9b94ebc5a7ed9002ea69a1c662592c4a06002',
        'encrypted_data':
            '',
      };

      await cryptoService.clearKeys();

      // Should return false gracefully without throwing a Null subtype cast crash
      final success = await cryptoService.unlockFromBlob(
        argon2idBlob,
        password,
      );
      expect(success, isFalse);
    ...
    test(
      'E2EE personal message encrypt and decrypt roundtrip between two users',
      () async {
        ...
        const originalMessage =
            "Hello Bob! This is Alice writing to you securely.";

        // Alice encrypts for Bob
        final encryptedB64 = await cryptoService.encryptPersonalMessage(
          originalMessage,
          bobPub,
          'personal_1_2',
        );
        ...
        final decryptedMessage = await cryptoService.decryptPersonalMessage(
          encryptedB64,
          alicePub,
          'personal_1_2',
        );
    ...
    test('Group message encrypt and decrypt roundtrip', () async {
      const chatKeyHex =
          "0102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20"; // 32-byte key
      const originalMessage = "Hello group members!";

      final encryptedB64 = await cryptoService.encryptGroupMessage(
        originalMessage,
        chatKeyHex,
      );
      final decrypted = await cryptoService.decryptGroupMessage(
        encryptedB64,
        chatKeyHex,
      );
